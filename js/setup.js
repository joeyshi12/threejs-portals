...
function createScene(canvas) {
  ...
  //const blueCam = new THREE.PerspectiveCamera(30.0, 1.0, 0.1, 1000.0);
  const blueCam = new THREE.OrthographicCamera(-window.innerWidth / denom, window.innerWidth / denom, window.innerHeight / denom, -window.innerHeight / denom, 1, 1000.0);
  scene.add(blueCam);

  //const orangeCam = new THREE.PerspectiveCamera(30.0, 1.0, 0.1, 1000.0);
  const orangeCam = new THREE.OrthographicCamera(-window.innerWidth / denom, window.innerWidth / denom, window.innerHeight / denom, -window.innerHeight / denom, 1, 1000.0);
  scene.add(orangeCam);
  
  const cubeRenderTarget = setupRenderTarget(true);
  const renderTarget = setupRenderTarget(true);
  const renderTarget2 = setupRenderTarget(true);
  const renderTarget3 = setupRenderTarget(true);
  ...
  function resize() {
    ...
    blueCam.left = -window.innerWidth / denom;
    blueCam.right = window.innerWidth / denom;
    blueCam.top = window.innerHeight / denom;
    blueCam.bottom = -window.innerHeight / denom;
    blueCam.updateProjectionMatrix();

    orangeCam.left = -window.innerWidth / denom;
    orangeCam.right = window.innerWidth / denom;
    orangeCam.top = window.innerHeight / denom;
    orangeCam.bottom = -window.innerHeight / denom;
    orangeCam.updateProjectionMatrix();
  }
